...
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% this code is part of paper entitled "Characterisation of neonatal cardiac dynamics using ordinal partition network"
...
% % ocurrence of each symbol in the symbolic dynamics S
par = unique(numb);
n_par = length(par);
pi = zeros(1,n_par);
for l=1:n_par
...
% finding the stochastic matrix P with elements
...
% estimation of the stationary distribution from matriz_adj
matriz_pi = sum(matriz_adj,2)/sum(sum(matriz_adj));

% conditional permutation entropy (h^(CPE))
...
% % % finding the NEW stochastic matrix P with elements: without self-loops
...
% % global node out-link entropy (GNE)
